...
# ইতিমধ্যে কোন শর্তের নোটিফিকেশন গেছে তা ট্র্যাক করতে
...
def send_telegram(message):
    ...
    try:
        ...
        if res.status_code == 200:
            print(f"  📨 Telegram মেসেজ পাঠানো হয়েছে!")
        else:
...
def build_alert_message(condition_name, detail, match_name, score, team2_name):
    """Telegram এ পাঠানোর মেসেজ তৈরি করে"""
    ...
    msg = (
        ...
        f"📊 <b>স্কোর:</b> {score}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"✅ <b>শর্ত পূরণ হয়েছে:</b>\n"
        f"   {condition_name}\n"
        f"   └─ {detail}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🎯 <b>{team2_name} (২য় ব্যাটিং টিম) জিততে পারে!</b>\n"
        f"⏰ {now}"
    )
    return msg
...
def parse_innings(scorecard):
    ...
    try:
        ...
        print(f"  [PARSE] ইনিংস এরর: {e}")
    return innings_list

def first_over_runs(inn):
    """১ম ওভারের রান বের করে"""
...
def cond3(innings_list):
    ...
    return None, f"২য় ইনিংস এখনো ১৬ ওভার হয়নি"
...
def cond5_6(innings_list, standings_data):
    ...
    try:
        for td in standings_data.get("standings", {}).get("standings", []):
            ...
            # শর্ত ৫: ২য় ব্যাটিং টিম Clear Topper?
            ...
            # শর্ত ৬: ১ম ব্যাটিং টিম Clear Bottom?
    ...
    try:
        for _, player in inn1["bat_details"].items():
            ...
            if "(wk)" in name.lower() or "†" in name or player.get("isKeeper", False):
                ...
                ok = not_out
                return ok, f"WK {name} {'আউট হননি ✅' if ok else 'আউট হয়ে গেছেন ❌'}"
...
def print_dashboard(match_name, score, results, team2_name):
    clear()
    print("=" * 65)
    print("  🏏  T20 LIVE CONDITION CHECKER  |  Telegram Alert Active")
    print(f"  ⏰  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  📍  {match_name}")
    print(f"  📊  {score}")
    print("=" * 65)
    passed = 0
    for i, (name, (val, detail)) in enumerate(zip(CONDITION_NAMES, results), 1):
        ...
        if val is True:
            passed += 1
    print("=" * 65)
    print(f"  ✅ পাস: {passed}/9  |  🎯 ২য় টিম: {team2_name}")
    print(f"  🔄 পরবর্তী আপডেট: {POLL_INTERVAL} সেকেন্ড পর | বন্ধ: Ctrl+C")
    print("=" * 65)
...
def main():
    ...
    print("  Cricbuzz (RapidAPI) + Telegram Alert | Python")
    print("=" * 65)

    # কনফিগ চেক
    errors = []
    if RAPIDAPI_KEY   == "YOUR_RAPIDAPI_KEY_HERE":   errors.append("RAPIDAPI_KEY")
    if TELEGRAM_TOKEN == "YOUR_TELEGRAM_BOT_TOKEN_HERE": errors.append("TELEGRAM_TOKEN")
    if TELEGRAM_CHAT_ID == "YOUR_CHAT_ID_HERE":      errors.append("TELEGRAM_CHAT_ID")

    if errors:
        print(f"\n  ⚠️  নিচের তথ্যগুলো বসাননি: {', '.join(errors)}")
        print("  ফাইলের উপরের কনফিগারেশন সেকশনে বসিয়ে আবার চালান।\n")
        return
